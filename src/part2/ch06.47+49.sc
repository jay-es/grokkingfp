def extractYearStart(rawShow: String): Either[String, Int] = {
  ...
  // yearStrEither
  //   .map(yearStr => yearStr.toIntOption.toRight(s"Can't parse $yearStr"))
  //   .flatten
...
val res1 = parseShows(List("The Wire (2002-2008)", "[2019]"))
val res2 = parseShows(List("The Wire (-)", "Chernobyl (2019)"))
val res3 = parseShows(List("The Wire (2002-2008)", "Chernobyl (2019)"))

assert(res1 == Left("Can't extract name from [2019]"))
assert(res2 == Left("Can't extract single year from The Wire (-)"))
assert(
  res3 == Right(
    List(
